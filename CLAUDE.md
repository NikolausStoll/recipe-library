...
- **Services**:
  ...
  - `recipeUrlExtractService.js` - Fetch recipe HTML, extract raw fields from JSON-LD Recipe + recipe-card HTML enrichment (ingredient groups, notes, instructions; no LLM); deduplicates `image_urls` that look like the same asset at different resolutions (keeps largest) before persisting to `image_urls_json`
  - `recipeHtmlEnrichment.js` - Recipe-card container detection and HTML parsing (WPRM, Tasty Recipes, Mediavine Create, microdata, heading/list fallbacks)
