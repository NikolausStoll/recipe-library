...
- **PWA config**: Keep `frontend/public/manifest.webmanifest` valid and linked from `frontend/index.html`; preserve `start_url`, `scope`, `theme_color`, `background_color`, and icon entries. Icon `sizes` must match actual file dimensions. Keep `frontend/public/sw.js` registered so Android Chrome can install as app.
